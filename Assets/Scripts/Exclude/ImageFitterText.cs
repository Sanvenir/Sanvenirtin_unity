...
public class ImageFitterText : MonoBehaviour
{
    ...
    [Tooltip("计算时加入Text的Scale")] public bool useTextScale = true;

    // Use this for initialization
    private void Start()
    {
        if (image == null) image = transform.GetComponent<Image>();
        if (image != null) imageRectTrans = image.GetComponent<RectTransform>();
        if (targetText == null) targetText = transform.GetComponentInChildren<Text>();
        if (targetText != null)
        {
    ...
    private Vector2 GetTextPreferredSize()
    {
        ...
        if (useTextScale) size = new Vector2(size.x * textScale.x, size.y * textScale.y);
        return size;
    }

    private void UpdateImageSize(Vector2 size, Vector2 offset)
    {
        if (imageRectTrans != null) imageRectTrans.sizeDelta = size + offset;
    }
...
public class ImageFitterTextEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        if (GUILayout.Button("Execute")) obj.Refresh();
